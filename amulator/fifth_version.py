...
def rmdir(node, names):
    for name in names:
        if name in node.data:
            # Удаляем из реальной файловой системы
            if node.data[name].real_path and os.path.exists(node.data[name].real_path):
                import shutil
                if node.data[name].file_type == 'dir':
                    shutil.rmtree(node.data[name].real_path)
                else:
                    os.remove(node.data[name].real_path)
            del node.data[name]
